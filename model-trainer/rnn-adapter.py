...
def run_disk_adapter():
   ...
   with open(result_output, "rb") as result_input:
    ...
    for row in csv_reader:
      ...
      utilityOfdisk= utilityOfdisk + ((anomalyLikelihood * sumOfUtilityFitness)/sumOfWeaight) * diffChange
      utilityOfdiskPredicted= utilityOfdiskPredicted + (anomalyLikelihood * sumOfUtilityFitnessPredicted)/sumOfWeaight * diffChange
    cost = 0 
    if diffChange > 0: 
...
def scaleCPU(cost):
    ...
    try:
      ...
      if len(cpuData) > 0:
        ...
        timestamp = datetime.datetime.fromtimestamp(float(cpuValue[0])).strftime('%m/%d/%y %H:%M')   
        cpu = 100 - float(cpuValue[1])
        print 'time: ', timestamp, 'irate: ', cost, ' cpu Usage: ',cpu 
        result = model_cpu_model.run({"cpu": cpu})
        prediction = result.inferences["multiStepBestPredictions"][1]
        anomalyScore = result.inferences['anomalyScore']
        anomalyLikelihood = anomalyLikelihoodHelper.anomalyProbability(cpu, anomalyScore, timestamp)
        sumOfUtilityFitness=   (float(cpu) * float(anomalyLikelihood))
        print 'sumOfUtilityFitness:=> ', sumOfUtilityFitness

      if sumOfUtilityFitness > 70.0 and cost >  0:
        print 'Scaling scaleCPU'
        filepath = os.path.join('/home/nupic/static-swarm-global/model-trainer/', 'cpu.sh')
        print filepath
        res= subprocess.call(filepath, shell=True)
        print res
      elif sumOfUtilityFitness < 25.0 or cost <= 0.0:
        print 'Scaling scaleCPU down'
        filepath = os.path.join('/home/nupic/static-swarm-global/model-trainer/', 'dec-cpu.sh')
        print filepath
        res= subprocess.call(filepath, shell=True)
        print res
...
def scaledisk():
  ...
  res= subprocess.call("/home/nupic/static-swarm-global/model-trainer/disk.sh", shell=True)
  print res
  pass
def callback():
...
def run():
  ...
  if adaptoutcome['metric'] == 'utilityOfMem':

    scaleMemeory()
    adaptoutcome['msg'] = 'scaleMemeory'
  elif adaptoutcome['metric'] == 'utilityOfCpu':
    scaleCPU(float(adaptoutcome['cost']))
    adaptoutcome['msg'] = 'scaleCPU'
  elif adaptoutcome['metric'] == 'utilityOfdisk':
    scaledisk()
